import { getTodayPhrase, type DailyPhrase } from "@/data/daily-phrases";
    ...
    <div className="mx-4 rounded-[14px] border border-[#EDEDED] bg-white p-4 shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#2E7D32]/10 text-sm">{"\uD83C\uDDFB\uD83C\uDDF3"}</span>
      ...
      <div className="rounded-[10px] bg-[#F2F1EF] px-3 py-2">
        <p className="text-xs text-[#999]">
          <span className="mr-1">{"\uD83D\uDCA1"}</span>{today.culture}
        </p>
      </div>
    </div>
  );

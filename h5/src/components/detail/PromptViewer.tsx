        ...
        <div className="mt-2 rounded-lg bg-[#1565C0]/5 p-3 space-y-3">
          <div className="rounded-md bg-white p-2.5 border border-[#1565C0]/15">
            <div className="flex items-center gap-1.5 mb-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-[#1565C0]/60">MODEL</span>
            </div>
            <p className="text-sm font-semibold text-[#111]">{prompt.model.name}</p>
            <p className="mt-0.5 text-xs text-[#666]">{prompt.model.reason}</p>
          </div>
          <div className="space-y-2">
            {layerEntries.map(([key, layer], index) => (
              <div key={key} className="flex gap-2">
                <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#1565C0]/10 text-[10px] font-bold text-[#1565C0]">{index + 1}</span>
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-semibold text-[#111]">{layer.label}</p>
                  <p className="mt-0.5 text-xs leading-relaxed text-[#666] break-words">
                    {layer.content.length > 120 ? layer.content.slice(0, 120) + "..." : layer.content}
                  </p>
                </div>
              </div>
            ))}

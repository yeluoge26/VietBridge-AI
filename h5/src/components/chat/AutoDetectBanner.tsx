...
const taskLabels: Record<string, string> = {
  translate: "翻译", reply: "回复建议", risk: "风险分析", learn: "教我说",
};
const sceneLabels: Record<string, string> = {
  general: "通用", business: "商务", staff: "员工管理", couple: "情侣",
  restaurant: "餐厅", rent: "租房", hospital: "医院", housekeeping: "家政",
};

export default function AutoDetectBanner({ task, scene, confidence, onApply, visible }: AutoDetectBannerProps) {
  ...
  return (
    ...
    >
      <span className="text-xs text-[#666]">
        ...
        <span className="ml-1 text-[#999]">(置信度{Math.round(confidence)}%)</span>
      </span>
      <span className="ml-2 shrink-0 text-xs font-semibold text-[#1565C0]">应用 &rarr;</span>
    </button>
  );
}

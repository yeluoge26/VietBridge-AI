...
export default function InputBar({ value, onChange, onSend, loading, taskColor, voiceLang = "zh-CN", onCamera }: InputBarProps) {
    ...
    <div className="border-t border-[#EDEDED] bg-white px-3 py-2 pb-[calc(8px+env(safe-area-inset-bottom))]">
      {listening && (
        <div className="mb-2 flex items-center gap-2 rounded-lg bg-red-50 px-3 py-1.5">
          ...
          <span className="text-xs text-red-600">{interim ? interim : "正在聆听..."}</span>
          <button type="button" onClick={toggle} className="ml-auto text-xs font-medium text-red-600 hover:text-red-800">停止</button>
        </div>
      )}
      ...
      <div className="flex items-end gap-2">
        {onCamera && (
          <button
            type="button"
            onClick={onCamera}
            className="flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-[10px] bg-[#F2F1EF] text-[#666] transition-all hover:bg-[#EDEDED] active:scale-[0.95]"
            aria-label="拍照翻译"
          >
        ...
        <div className="relative flex-1">
          ...
          {supported && (
            <button
              type="button"
              onClick={toggle}
              className={`absolute bottom-2 right-2 flex h-7 w-7 items-center justify-center rounded-full transition-all ${listening ? "bg-red-500 text-white scale-110" : "text-[#999] hover:bg-[#EDEDED] hover:text-[#666]"}`}
        ...
        <button
          ...
          style={{ backgroundColor: hasText ? taskColor : "#EDEDED" }}
        >
          {loading ? (
            <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" />
          ) : (
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={hasText ? "#FFF" : "#999"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">

...
export default function ScanModal({ open, onClose }: ScanModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#F8F7F5]">
      {/* Header */}
      <div className="flex items-center justify-between px-4 pt-[env(safe-area-inset-top)] pb-2 border-b border-[#EDEDED] bg-white">
        <div className="py-2">
          <h1 className="text-lg font-bold text-[#111]">拍照翻译</h1>
          <p className="text-xs text-[#999]">拍照或上传文档，AI帮你分析</p>
        </div>
        <button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-full hover:bg-[#F2F1EF] transition-colors">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-8">
              ...
              <div className="grid grid-cols-3 gap-2">
                {DOC_TYPES.map((dt) => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-1.5 rounded-xl border p-3 transition-all ${docType === dt.id ? "border-[#111] bg-[#111] text-white" : "border-[#EDEDED] bg-white text-[#111] hover:bg-[#F8F7F5]"}`}
                  >
        ...
        {step === "ocr" && (
          <div className="flex flex-col items-center gap-4 pt-8">
            {imageUrl && <img src={imageUrl} alt="扫描图片" className="h-48 w-auto rounded-xl border border-[#EDEDED] object-cover" />}
            <div className="w-full max-w-xs">
              ...
              <div className="h-2 overflow-hidden rounded-full bg-[#EDEDED]">
                <div className="h-full rounded-full bg-[#111] transition-all duration-300" style={{ width: `${ocrProgress}%` }} />
              </div>
            </div>
          </div>
        )}
        ...
        {step === "result" && (
          <div className="space-y-4 pt-4">
            {imageUrl && <img src={imageUrl} alt="扫描图片" className="h-32 w-auto rounded-xl border border-[#EDEDED] object-cover" />}

            {ocrText && (
              <div className="rounded-xl border border-[#EDEDED] bg-white p-4">
                <p className="mb-2 text-xs font-semibold text-[#999]">识别文字</p>
                <p className="max-h-32 overflow-y-auto text-sm text-[#666] whitespace-pre-wrap">{ocrText}</p>
              </div>
            )}
            ...
            {result && (
              <div className="rounded-xl border border-[#EDEDED] bg-white p-4 space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-[#111]">{DOC_TYPES.find((d) => d.id === docType)?.label}分析结果</span>
                </div>
                    ...
                    {result.data.items.map((item: { name?: string; name_vi?: string; name_zh?: string; price?: string; unit?: string; note?: string; priceReasonable?: boolean }, i: number) => (
                      <div key={i} className={`flex items-center justify-between rounded-lg px-3 py-2 ${item.priceReasonable === false ? "bg-red-50" : "bg-[#F2F1EF]"}`}>
                        <div className="min-w-0 flex-1">
                          ...
                          {item.name_vi && item.name_zh && <span className="ml-1 text-xs text-[#999]">({item.name_vi})</span>}
                        </div>
                        {item.price && (
                ...
                {result.data?.warnings && Array.isArray(result.data.warnings) && result.data.warnings.length > 0 && (
                  <div className="space-y-1">
                    ...
                    {result.data.warnings.map((w: string, i: number) => (
                      <div key={i} className="rounded-lg bg-orange-50 px-3 py-2"><p className="text-xs text-orange-700">{w}</p></div>
                    ))}
                  </div>
                )}
                ...
                {result.data?.tips && Array.isArray(result.data.tips) && result.data.tips.length > 0 && (
                  <div className="space-y-1">
                    ...
                    {result.data.tips.map((tip: string, i: number) => <p key={i} className="text-xs text-[#666]">{i + 1}. {tip}</p>)}
                  </div>
                )}
              </div>
            )}
            ...
            <button onClick={handleReset} className="flex w-full items-center justify-center gap-2 rounded-xl border border-[#DDD] bg-white py-3 text-sm font-medium text-[#111] transition-colors hover:bg-[#F8F7F5]">重新扫描</button>
          </div>
        )}
      </div>
    </div>

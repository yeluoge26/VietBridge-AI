...
${tone <= 3 ? "使用非常口语化、轻松的表达方式。可以使用缩写和俚语。" : ""}${tone >= 4 && tone <= 6 ? "使用标准日常表达，保持自然但不过于随意。" : ""}${tone >= 7 ? "使用正式、礼貌的表达。句尾加敬语助词。避免俚语。" : ""}`,
  ...
  // Layer 6: Knowledge - RAG knowledge base hits
  const knowledgeLayer: Layer = {
    label: "知识层",
    content:
      kbHits && kbHits.length > 0
        ? `以下是与用户输入相关的知识库参考信息（请在回复中参考这些数据，但不要直接照搬）：\n${kbHits
            .map(
              (h, i) =>
                `${i + 1}. [${h.source}, 置信度${Math.round(h.confidence * 100)}%] ${h.detail}`
            )
            .join("\n")}`
        : "无相关知识库命中",
  };

  // Layer 7: Context - Conversation history
  ...
  // Layer 8: Input - User's actual input
...
export function flattenToMessages(
  ...
  // Include knowledge layer if it has hits
  if (layers.knowledge.content !== "无相关知识库命中") {
    systemParts.push(layers.knowledge.content);
  }

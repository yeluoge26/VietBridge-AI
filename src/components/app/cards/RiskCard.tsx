...
export default function RiskCard({
  ...
  // Sort factors by weight descending for visual impact
                  ...
                  <div className="h-1.5 w-full rounded-full bg-[#F2F1EF]">
                    <div
                      className={`h-full rounded-full transition-all duration-500 ${
                        factor.active ? barColor : "bg-[#EDEDED]"
                      }`}
            ...
            <ol className="space-y-1.5">
              {data.tips.map((tip, index) => (
                <li key={index} className="flex gap-2">
                  <span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-[#FF8A00]/10 text-[10px] font-bold text-[#FF8A00]">
                    {index + 1}
                  </span>
          ...
          <div>
            ...
            <div className="space-y-2">
              {data.scripts.map((script, index) => (
                <div
                  key={index}
                  className="rounded-lg border border-[#EDEDED] p-3"
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => onCopy(script.vi)}
                      className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#F2F1EF] transition-colors hover:bg-[#EDEDED]"
                      aria-label="复制话术"
                    >
                      <svg
                        width="14"
